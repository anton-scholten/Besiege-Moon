...
#
# The folder Besiege loads is Moon/, not the repository root: that subfolder is
# the whole of what gets uploaded to the Workshop, and everything beside it --
# sources, tools, docs, working files -- is not part of the mod.
...
if [[ ! -f "$SRC/MoonAssembly.dll" ]]; then
    echo "Moon/MoonAssembly.dll is missing; run ./tools/build.sh first." >&2
    exit 1
fi
...
Done. Next:
  1. Start Besiege and enable "Moon" in the mods menu.
  2. The Gravity Gun and Moon blocks appear in the block menu; search "moon"
     or "gravity gun".
  3. The atmosphere is off by default. Turn it on from the mod console with
     `atmosphere true`, and set its two altitudes with `minAltitude` and
     `maxAltitude`.
