#!/usr/bin/env bash
#
# Compiles the mod into MoonAssembly.dll, using Besiege's OWN C# compiler rather
...
#
# Mod.xml loads MoonAssembly.dll directly, so the mod ships as a prebuilt
...
SRC_DIR="$REPO_DIR/Moon/MoonScripts"
BUILD_DIR="${TMPDIR:-/tmp}/besiege-moon-build"
OUT="$REPO_DIR/Moon/MoonAssembly.dll"
...
mkdir -p "$BUILD_DIR"
TMP_OUT="$BUILD_DIR/MoonAssembly.$$.dll"
...
# System.Xml is referenced for the [Xml*] attributes on the block modules. That
